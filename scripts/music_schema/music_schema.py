...
train_path = 'data/music_schema/music-train.pkl'
valid_path = 'data/music_schema/music-valid.pkl'
test_path = 'data/music_schema/music-test.pkl'
...
# def adagrad(param, rate, epsilon, gradient, updates, param_squared_gradients):
cmd_adagrad = ('./learn_parameters.py --seed=%d --strategy=%s --totepochs=%d --test_all=%d --lr=%f --name=music_schema/music_%s_%d '
                ' --train=%s --valid=%s --test=%s --nbatches=%d --no_rescaling --filtered '
                ' --domain_range=data/music_schema/music_domains_ranges.pkl '
